...
interface DataTableTreeProps {
  ...
  onLoadChildren?: (parentRow: any) => Promise<any[]>;
  sortField?: string;
...
function loadSettings(storageKey: string): StoredSettings | null {
  try {
    const stored = localStorage.getItem(`dataTableTree_${storageKey}`);
    if (stored) {
...
function saveSettings(storageKey: string, settings: StoredSettings) {
  try {
    localStorage.setItem(`dataTableTree_${storageKey}`, JSON.stringify(settings));
  } catch (e) {
  }
}
...
function DataTableTree({
  ...
  const handleToggleNode = async (nodeIndex: number) => {
    ...
    if (!node.isExpanded && !node.childrenLoaded && onLoadChildren) {
      ...
      try {
        const children = await onLoadChildren(node.row);
  ...
  const updateNodeChildren = (parentNode: TreeNode, childrenData: any[]) => {
    setTreeNodes(prev => {
      const newNodes = [...prev];
      const updateNode = (nodes: TreeNode[]): boolean => {
        for (const node of nodes) {
          if (node.row[itemKeyId] === parentNode.row[itemKeyId]) {
            node.children = childrenData.map(childRow => ({
              row: childRow,
              level: node.level + 1,
              isExpanded: false,
              hasChildren: node.level + 1 < maxDeepLevel,
              childrenLoaded: false,
              children: [],
            }));
  ...
  const toggleNodeExpansion = (targetNode: TreeNode) => {
    setTreeNodes(prev => {
      const newNodes = [...prev];
      const toggleNode = (nodes: TreeNode[]): boolean => {
        for (const node of nodes) {
          if (node.row[itemKeyId] === targetNode.row[itemKeyId]) {
        ...
        <table className="w-full" style={{ minWidth: 'max-content' }}>
          ...
          >
            ...
            <tr>
              ...
              >
                {/* Колонка для иконки развертывания */}
              </th>
          ...
          <tbody>
            {loading ? (
              ...
              flatData.map((node, idx) => (
                <tr
                  key={`${node.row[itemKeyId]}-${idx}`}
                ...
                >
                  ...
                  {visibleColumns.map((col) => (
                    ...
                    >
                      {col.render ? col.render(node.row[col.key], node.row) : formatCellValue(node.row[col.key])}
                    </td>
                  ))}
                </tr>
              ))
